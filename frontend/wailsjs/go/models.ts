export namespace docparse {
  export class DocsTreeNode {
    Name: string;
    IsMdFile: boolean;
    ...
    static createFrom(source: any = {}) {
      return new DocsTreeNode(source);
    }

    constructor(source: any = {}) {
      if ("string" === typeof source) source = JSON.parse(source);
      this.Name = source["Name"];
      this.IsMdFile = source["IsMdFile"];
      this.MdFileFrontMatter = source["MdFileFrontMatter"];
      this.Path = source["Path"];
      this.FullPath = source["FullPath"];
      this.Children = this.convertValues(source["Children"], DocsTreeNode);
    }
