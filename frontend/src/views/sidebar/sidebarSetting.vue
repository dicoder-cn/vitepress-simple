      ...
      <a-popconfirm
        ...
        <q-tooltip class="text-2xl">
          ...
          <span class="text-cyan-500">{{
            isUseManySidebars ? lang('pageSidebar.multiSidebar') : lang('pageSidebar.singleSidebar')
          }}</span>
...
<script setup lang="ts">
  ...
  const setCurrTreeData = async () => {
    ...
    if (isUseManySidebars.value) {
      ...
      sidebarTree.value =
        storeConfig.currLangConfig['themeConfig']['sidebar'][currSelectSidebarKey.value]
    } else {
      //单侧栏模式
  ...
  const recognitionSidebar = async () => {
    ...
    let baseDir = await PathJoin([storeConfig.srcDir, storeConfig.currSettingLang])
    ...
    if (!treeData) {
      ...
      let existingLinks: string[] = sidebarTree.value.flatMap((nav) =>
        nav.items ? [nav.link, ...nav.items.map((item) => item.link)] : [nav.link]
      )
    ...
    treeNodes.forEach((treeNode) => {
      ...
      if (!existingLinks.includes(vpNavItem.link)) {
        ...
        if (treeNode.children) {
          vpNavItem.items = convertTreeNodesToVpNavList(treeNode.children, existingLinks).filter(
            Boolean
          )
        }
      }
