<template>
  ...
  <div class="ml-5 mt-5 flex justify-start">
    ...
    <config-switch
      ...
      :config-key="ConfigKeyIsStartup"></config-switch>
  </div>
  <div class="ml-5 mt-5">
    <config-radio
  ...
  <div class="flex items-center justify-center">
    <icon-btn
      ...
      :text="lang('pageSetting.settingBase.checkUpdate')"></icon-btn>
    <icon-btn
      @click="openDataDir()"
      btn-class="bg-orange"
      icon="seo-folder"
      :text="lang('pageSetting.settingBase.openDataDir')"></icon-btn>
  </div>
</template>
