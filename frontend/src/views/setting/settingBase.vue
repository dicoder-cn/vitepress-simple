<template>
  ...
  <div class="flex items-center justify-center">
    <icon-btn class="mx-2" @click="checkUpdate()" icon="update-rotation" :text="lang('pageSetting.settingBase.checkUpdate')"></icon-btn>
    <icon-btn @click="openDataDir()" btn-class="bg-orange" icon="seo-folder" :text="lang('pageSetting.settingBase.openDataDir')"></icon-btn>
  </div>
</template>
<script setup lang="ts">
import { ConfigKeyChangeAutoSave, ConfigKeyFrontMatterSaveType, ConfigKeyIsStartup, ConfigKeySysUpdateSource } from "@/configs/appConfigKey";
import { useLayoutStore } from "@/store/layout";
import { GetSystemUserHomeDir, OpenFileBrowser, PathJoin } from "../../../wailsjs/go/system/SystemService";
