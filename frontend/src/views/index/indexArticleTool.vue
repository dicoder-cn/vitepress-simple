  ...
  <div class="px-2 py-2 border-b" v-show="!storeIndex.IsEmptyProject">
    <div class="flex items-center justify-start" style="overflow-y: hidden">
      <div class="mx-1 cursor-pointer">
        ...
        <a-dropdown arrow>
          <a class="ant-dropdown-link" @click.prevent>
            <icon-park :size="iconSize" fill="black" strokeLinejoin="bevel" theme="outline" type="add" />
          </a>
          <template #overlay>
            <a-menu>
      ...
      <div class="mx-1 cursor-pointer" @click="openProject()">
        ...
        <a class="ant-dropdown-link" @click.prevent>
          <icon-park :size="iconSize" fill="#333" strokeLinejoin="bevel" theme="outline" type="layers" />
        </a>
      </div>
      <div class="mx-1 cursor-pointer" @click="OpenFileBrowser(storeVpConfig.baseDir)">
        <q-tooltip>{{ lang("pageIndex.openInFileBrowser") }}</q-tooltip>
        <icon-park :size="iconSize" fill="#333" strokeLinejoin="bevel" theme="outline" type="folder-close" @click="storeIndex.loadTreeData()" />
      </div>
      <div class="mx-1 cursor-pointer">
        <q-tooltip>{{ lang("pageIndex.refreshDirectoryTree") }}</q-tooltip>
        <icon-park :size="iconSize" fill="#333" strokeLinejoin="bevel" theme="outline" type="refresh" @click="storeIndex.loadTreeData()" />
      </div>
    </div>
    ...
    <input-model ref="refModalCreateDir" :placeholder="lang('pageIndex.inputFolderName')" :title="lang('pageIndex.inputFolderName')" @submit-input-modal="onSubmitInputModalCreateDir"></input-model>
    <input-model
...
import { CreateDir, CreateFile } from "../../../wailsjs/go/services/ArticleTreeData";
...
const onSubmitInputModalCreateDir = async (value: string) => {
  ...
  if (ToastCheck(res, lang("pageIndex.folderCreated"))) await storeIndex.loadTreeData();
};
