<template>
  <div class="flex justify-start items-center my-2 mx-8">
    ...
    <div class="my-3 w-1/3 flex justify-between">
      <div class="flex-1">
        <a-input disabled v-model:value="storeConfig.currLangConfig?.themeConfig?.logo" :placeholder="lang('pageProject.settingBase.placeholders.logoUrl')" class="w-full"> </a-input>
      </div>
    ...
    <sim-input
      v-model="storeConfig.currLangConfig?.themeConfig?.outline"
    ...
    <sim-switch
      v-model="storeConfig.currLangConfig?.themeConfig?.externalLinkIcon"
    ...
    <sim-switch v-model="storeConfig.vpConfig?.cacheDir" :tooltip="lang('pageProject.settingBase.tooltips.cleanUrls')" label="cleanUrls"></sim-switch>
    <!--    <sim-switch-->
