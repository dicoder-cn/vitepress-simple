<template>
  <div id="components-modal-demo-position">
    <a-modal
      ...
      @ok="OpenDir">
      <div class="mr-4 my-2">
        <a-alert closable :description="lang('pageProject.errorProjectNotVPSimple')" type="success" />
      </div>
        ...
        <div>
          <a-list item-layout="horizontal" :data-source="data">
            <template #renderItem="{ item }">
              <a-list-item>
                <template #actions>
                  <a-button @click="openProject(item.title)" class="bg-blue-500 text-white">{{ lang("common.open") }} </a-button>
                </template>
                <a-list-item-meta>
                  <template #title>
