<template>
  <div class="flex justify-start items-center ml-2" v-if="storeConfig.IsUseI18n">
    ...
    当前操作的语言：
    <a-radio-group @change="langChange" v-model:value="storeConfig.currSettingLang">
      ...
      <a-radio-button v-for="(item, index) in storeConfig.GetLangArray" :key="index" :value="item"
        >{{ item }}
      </a-radio-button>
    </a-radio-group>
  </div>
