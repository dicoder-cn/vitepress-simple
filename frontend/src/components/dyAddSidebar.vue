<template>
  <div>
    ...
    <div v-for="(item, index) in SidebarArray" :key="index">
      <div class="flex mt-1 justify-start items-center" :style="marginLeft">
        ...
        <div class="mt-1 px-1">
          <a-tooltip title="目前最高支持到第三级侧栏">
            <a-button :disabled="props.level == 3" type="primary" ghost @click="addSubNav(index)"
              >添加子级
            </a-button>
          </a-tooltip>
        </div>
      ...
      <div
        v-if="SidebarArray[index].items && SidebarArray[index].items.length > 0 && isExpand(index)">
        <dy-add-sidebar
          :level="props.level + 1"
          v-model:sidebar-array="SidebarArray[index].items"></dy-add-sidebar>
      </div>
    </div>
  </div>
</template>
