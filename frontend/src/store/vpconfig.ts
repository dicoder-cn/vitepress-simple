...
import { CopyPath, PathExists, PathJoin } from '../../wailsjs/go/system/SystemService'
...
export const useVpconfigStore = defineStore('vpconfig', {
        ...
        if (key === 'themeConfig') {
          ...
          for (const key2 in defaultShareConfigValue.themeConfig) {
            if (IsEmptyValue(this.configData[key][key2])) {
              this.configData[key][key2] = defaultShareConfigValue.themeConfig[key2]
            }
          }
        }
      }
  ...
  getters: {
    ...
    IsUseI18n: (state) => {
      return (
        !IsEmptyValue(state.configData.locales) && Object.keys(state.configData.locales).length > 0
      )
    },
